...
final class WC_PayToday_Gateway_Blocks_Support extends AbstractPaymentMethodType {
	
	private $gateway;
	
	protected $name = 'paytoday'; // payment gateway id
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			'wc-paytoday-blocks-integration',
		...
		return array( 'wc-paytoday-blocks-integration' );
	...
	//	$sandbox_mode   = ( ! empty( $this->settings[ 'sandbox_mode' ] ) && 'yes' === $this->settings[ 'sandbox_mode' ] );
	//	$setting_key = $sandbox_mode ? 'test_publishable_key' : 'publishable_key';
